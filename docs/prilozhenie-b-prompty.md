...
**Версия:** 1.2 (этап 6: полный отчёт генерируется по частям, раздел 5а)
...
- Выход — строго **валидный JSON по схеме** (см. разделы 5 и 7). На бэкенде — валидация; при несоответствии — регенерация (до 2 ретраев; для тизера — 1 повтор с подсказкой, что исправить, см. раздел 4).
...
**Сообщение для повторной попытки** (этап 4б). Если ответ ИИ не прошёл проверку, делается **не больше одного** повтора. В нём ИИ получает свой прошлый ответ и это сообщение со списком найденных проблем, чтобы исправить именно их, а не писать заново вслепую. `{{problems}}` — список проблем, по одной на строку.

```
Ваш предыдущий ответ не прошёл проверку. Что нужно исправить:
{{problems}}
...
## 5а. Генерация полного отчёта по частям (этап 6)

Полный отчёт — длинный текст (особенно «Навигатор» на Opus и на узбекском), и одним ответом он может не уложиться в лимит времени на запрос у хостинга. Поэтому отчёт пишется **в 3 части**, по очереди: каждая часть — отдельный вызов ИИ и отдельный запрос к серверу. Системный промпт у всех частей один и тот же (кэшируется), меняется только пользовательское сообщение.

| Часть | Код | Поля схемы (раздел 7) |
|---|---|---|
| 1 | `portrait_goal` | `portrait`, `goal`, `reality_check` |
| 2 | `main_path` | `main_path.summary`, `main_path.routes` |
| 3 | `finish` | `main_path.learning_advice`, `main_path.future_outlook`, `alternatives`, `act_now`, `disclaimer` |

Каждая следующая часть получает уже готовые части, чтобы держаться их (цель из части 1 → маршрут в части 2 → альтернативы и шаги в части 3). Готовые части склеиваются в один JSON по схеме раздела 7 и проверяются правилами раздела 8.

Проверка каждой части — схемой и правилами (раздел 8, решения (З) и (П)); при провале — **1 повтор** с сообщением ниже (как у тизера). Если и повтор не прошёл, человек видит понятное сообщение и кнопку «Сгенерировать заново» — без повторной оплаты; уже готовые части при этом не пишутся заново.

`temperature` ~0.6 (передаётся только моделям, которые её принимают), глубина размышлений `effort: medium`, лимит ответа — на каждую часть отдельно.

**Системный промпт Вызова 2** собирается так (для `uz` в конце добавляется блок раздела 3а):

```
[СИСТЕМНЫЙ ПРОМПТ ИЗ РАЗДЕЛА 5]
...
Затем — строка о языке из раздела 3. Уровень и тип пути в системной части не подставляются: вместо `{{level}}` и `{{path_type}}` стоят слова «см. «Уровень» в сообщении» и «см. «Тип пути» в сообщении», а сами значения идут в пользовательском сообщении (строка «Уровень: … | Тип пути: …» из раздела 5). Так системная часть одна на весь сайт для каждого языка и кэшируется (prompt caching) для всех частей и всех пользователей.

**Пользовательское сообщение части** — шаблон из раздела 5, к которому через пустую строку добавляется:

```
ЧАСТЬ ОТЧЁТА {{part_number}} ИЗ {{part_total}}.
...
Для первой части вместо `{{previous_parts_json}}` подставляется `(пока нет — это первая часть)`.

**Задача части 1** (`portrait_goal`):

```
Опишите портрет человека, сформулируйте цель (точку Б) и честно сверьте её с профилем (reality_check).
Все следующие части строят маршрут именно к этой цели.
```

К ней добавляется правило по типу пути (решения (В) и (З)):

- `knows_goal`:

```
Тип пути knows_goal: у человека есть своя цель. goal.source = "stated", goal.statement — его цель,
сформулированная измеримо; goal.constructed_options можно оставить пустым; reality_check обязателен.
```

- `no_goal`, уровень `navigator`:

```
Тип пути no_goal: своей цели у человека нет. goal.source = "constructed", в goal.constructed_options —
...
- `no_goal`, уровень `route` (человек без цели купил «Маршрут»):

```
Тип пути no_goal, но выбран уровень route: постройте маршрут к первому направлению из fitting_directions
...
**Задача части 2** (`main_path`):

```
Постройте главный маршрут к цели из первой части: суть маршрута (main_path.summary) и маршруты
...
**Задача части 3** (`finish`):

```
Завершите отчёт: как именно человеку учиться под его стиль восприятия (main_path.learning_advice),
...
**Сообщение для повторной попытки части.** ИИ получает свой прошлый ответ и список проблем:

# Эталонные узбекские тексты (образец стиля для ИИ)

## Как это работает

Всё, что написано ниже под заголовками `## Namuna ...`, дословно подставляется в узбекский промпт ИИ как **образец стиля**. ИИ получает прямую инструкцию брать отсюда только тон и манеру, но **не копировать** факты, названия типов и направления.

Как менять:

- чтобы заменить образец, сотрите текст под заголовком `## Namuna ...` и вставьте свой;
- чтобы добавить образец, добавьте новый заголовок `## Namuna 3` (номер любой) и текст под ним;
- чтобы убрать образец, удалите заголовок вместе с текстом;
- формат свободный: обычный текст, как его увидит человек. JSON не нужен;
- апострофы в oʻ и gʻ можно писать любым знаком (`'`, `‘`, `ʻ`).

Всё, что стоит выше первого заголовка `## Namuna`, в промпт не попадает.

Принципы стиля описаны в [`uz-style.md`](uz-style.md).

## Namuna 1

Автор: носитель языка (владелец), сентябрь 2026. Первая часть тизера: тип, портрет, подходящие направления.

Sizning yo‘nalishingiz — Pragmatist-tadbirkor
Siz amaliy fikrlaydigan, mustaqil qaror qilishni yaxshi ko‘radigan odamsiz. Tayyor yo‘ldan yurishdan ko‘ra, o‘z yo‘lingizni topish sizga yaqinroq.
Tajribangiz shuni ko‘rsatadiki, siz sharoitga tez moslasha olasiz, imkoniyatni ko‘rsangiz esa uni sinab ko‘rishdan qo‘rqmaysiz. Pul siz uchun shunchaki daromad emas — u ko‘proq erkinlik, tanlov va o‘z ishingizni qurish imkonini anglatadi.
Sizga ayniqsa o‘z g‘oyangizni hayotga tatbiq qilish, natijani o‘z qo‘lingiz bilan yaratish va qilgan ishingizning samarasini ko‘rish muhim.
Sizga mos yo‘nalishlar

1. Onlayn biznes va e-commerce. Amaliy tajribangiz va texnologiyaga qiziqishingiz internet orqali biznes qurish uchun yaxshi asos bo‘la oladi.
2. Konsalting va biznes maslahati. Muammoni tez tushunish, yechim topish va boshqalarga yo‘l ko‘rsatish — kuchli tomonlaringizdan biri.
3. Xalqaro startaplar. Mustaqillikka intilishingiz, tavakkalni hisobga olib qaror qilishingiz va bir nechta tilni bilishingiz xalqaro loyihalarda sizga qo‘l kelishi mumkin.

## Namuna 2

Автор: носитель языка (владелец), сентябрь 2026. Вторая часть тизера: крючок про неожиданное направление.

Sizni hayron qoldirishi mumkin bo‘lgan yo‘nalish
Test natijalarida yana bir qiziq yo‘nalish ko‘rindi. Bir qarashda u sizga mos emasdek tuyulishi mumkin. Lekin javoblaringizda odamlar bilan ishlash, ularni tushunish va murakkab vaziyatlarda yo‘l ko‘rsata olish qobiliyati seziladi.
Bu qaysi yo‘nalish ekani va nega aynan sizga mos kelishi — to‘liq hisobotda.
